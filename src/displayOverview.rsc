...
real radius = sqrt(pow(0.5, 2)+ pow(0.5,2));  // distance betwn (0.0,0.0)  (0.5,0.5), center is .5,.5
...
public Figure cduplication(){
  ...
  list[tuple[real persc, str colors]] percnt = [<20.0,"red">, <80.0,"lightgreen">];
  return makeCircle(percnt, overview.ovShrink, overview.ovColors);
}
...
public Figure makeCircle(percnt, ovShrink, ovColors){
  Figure point(num x, num y){ return ellipse(shrink(0.0), align(x,y), fillColor(ovColors));}
// radius = .7071
